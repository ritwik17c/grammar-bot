...
LOGO_PATH = "vkv_logo.jpg"
VKV_HEADER = "🏫 *VKV Nalbari Grammar Assistant*\n━━━━━━━━━━━━━━━━━━━━━\n"
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS banned_users (
        user_id INTEGER PRIMARY KEY, username TEXT, banned_at TEXT, reason TEXT)""")
...
def ban_user(user_id, username, reason=""):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO banned_users (user_id, username, banned_at, reason) VALUES (?,?,?,?)",
...
def add_bot_admin(user_id, username):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("INSERT OR IGNORE INTO admins (user_id, username, added_at) VALUES (?,?,?)",
              (user_id, username, datetime.now().isoformat()))
    conn.commit()
    conn.close()
...
def get_all_users():
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("""SELECT k.user_id, k.username, k.first_seen,
                 COUNT(co.id) as total_corrections,
                 CASE WHEN b.user_id IS NOT NULL THEN 1 ELSE 0 END as is_banned
                 FROM known_users k
                 LEFT JOIN corrections co ON k.user_id = co.user_id
                 LEFT JOIN banned_users b ON k.user_id = b.user_id
                 GROUP BY k.user_id ORDER BY k.first_seen DESC""")
    users = c.fetchall()
    conn.close()
    return users
...
# Get INVITE_CODE from environment (optional - leave empty to disable)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check invite code if set
    if INVITE_CODE:
        args = context.args
        if not args or args[0] != INVITE_CODE:
            await update.message.reply_text(
                "🔒 This bot requires an invitation code to use.\n"
                "Please contact the group administrator to get access."
            )
            return

    add_known_user(user.id, user.username or user.first_name)
    # Auto-register as bot admin if first user
...
async def users_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not is_bot_admin(user.id):
        await update.message.reply_text("⚠️ Only bot administrators can view users.")
        return
    users = get_all_users()
    if not users:
    ...
    for uid, username, first_seen, corrections, banned in users:
        name = f"@{username}" if username else f"ID:{uid}"
        status = "🚫 Banned" if banned else "✅ Active"
        date = first_seen[:10] if first_seen else "Unknown"
        text += f"{status} {name}\n  Joined: {date} | Corrections: {corrections}\n\n"
...
async def ban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not is_bot_admin(user.id):
        await update.message.reply_text("⚠️ Only bot administrators can ban users.")
        return
    if not context.args:
        await update.message.reply_text("Usage: /ban <user_id or @username> <reason(optional)>\nExample: /ban 123456789 Misuse")
        return
    ...
    c = conn.cursor()
    if target.isdigit():
        c.execute("SELECT user_id, username FROM known_users WHERE user_id=?", (int(target),))
    else:
        c.execute("SELECT user_id, username FROM known_users WHERE username=?", (target,))
    row = c.fetchone()
    conn.close()
    if not row:
        await update.message.reply_text(f"⚠️ User '{target}' not found in database.\nMake sure they have used the bot at least once.")
        return
    ban_user(row[0], row[1], reason)
    await update.message.reply_text(
        f"🚫 User @{row[1] or row[0]} has been *banned*.\nReason: {reason}",
        parse_mode="Markdown"
    )
...
async def unban_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user
    if not is_bot_admin(user.id):
        await update.message.reply_text("⚠️ Only bot administrators can unban users.")
        return
    if not context.args:
    ...
    conn.close()
    if not row:
        await update.message.reply_text(f"⚠️ User '{target}' not found.")
        return
    unban_user(row[0])
    await update.message.reply_text(
        f"✅ User @{row[1] or row[0]} has been *unbanned* and can use the bot again.",
        parse_mode="Markdown"
    )
